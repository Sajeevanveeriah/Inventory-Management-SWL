...
flowchart LR
  ...
  Overview --> Runs[Runs]
  Catalogue --> Inventory
  Catalogue --> Expansion[Expansion catalogue]
  Catalogue --> Suppliers[Products and suppliers]
  Catalogue --> MappingProfiles[Saved import layouts]
  Pricing --> PricingRules[Pricing rules]
  Pricing --> Competitors[Competitor search]
  Pricing --> Sources[Price sources]
  Review --> Exceptions[Exceptions]
  Review --> Approvals[Approvals]
  Review --> Exports[Exports]
  System --> Integrations[Connect systems]
  System --> Audit[Audit]
...
all 17 surfaces. The global product search opens Find a product. New run contains the seven
...
records and price-source access remain available.
...
| Routes              |                                                                 17 | Hash route table and grouped navigation in `src/ui/routes.ts` |
