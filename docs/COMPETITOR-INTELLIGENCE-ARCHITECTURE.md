...
| Serper Shopping AU    | enabled only when explicitly configured      | API key                             | Finite provider credits and terms apply              |
| eBay Browse AU        | enabled only when explicitly configured      | OAuth application credentials       | Official API quotas and `EBAY_AU` terms apply        |
