    ...
    <section className="card" aria-labelledby="recovery-title">
      ...
      {restorePreview && (
        <div className="callout callout-warn" role="status">
          ...
          <p>
            {restorePreview.filename}, schema {restorePreview.schemaVersion},
            application {restorePreview.applicationVersion}. Integrity check:{" "}
            {restorePreview.integrityOk ? "passed" : "failed"}.
          </p>
          <p className="small">
            Records: {restorePreview.recordCounts.catalogueItems} catalogue
            items, {restorePreview.recordCounts.approvals} approvals,{" "}
            ...
            {restorePreview.recordCounts.aliases} aliases. SHA-256:{" "}
            {restorePreview.sha256}.
          </p>
          <label>
            <input
              type="checkbox"
            ...
            />{" "}
            I understand that restore replaces the current authorised local data
            after creating a verified pre-restore backup.
          </label>
          <div className="btn-row">
            <button
              type="button"
            ...
            <button
              ...
              disabled={
                working || !restoreConfirmed || !restorePreview.integrityOk
              }
...
export function SettingsPage({
  ...
  const [migration, setMigration] =
    useState<ConfigurationMigrationStatus | null>(null);
  const [message, setMessage] = useState("");
  const [working, setWorking] = useState(false);
  ...
  useEffect(() => {
    ...
    void platform.configuration.migrationStatus().then((result) => {
      ...
      if (!result.ok) {
        setMessage(
          `Legacy configuration inspection failed. ${result.error.message}`,
        );
        return;
      }
  ...
  const exportConfiguration = async () => {
    ...
    if (platform.kind === "desktop") {
      const saved =
        await platform.configuration.exportToSelectedFolder(filename);
      setWorking(false);
      if (!saved.ok) {
  ...
  const previewImport = async (serialised = draft) => {
    ...
    setMessage(
      "Preview validated. No live data has changed. Review counts and conflicts below.",
    );
  };
  ...
  const previewLegacy = async () => {
    ...
    setPreview(result.value);
    setMessage(
      "Legacy WebView configuration validated. No live data has changed.",
    );
  };
  ...
  const applyImport = async () => {
    if (!preview) return;
    setWorking(true);
    const result = await platform.configuration.applyImport(
  ...
  return (
    ...
    >
      <section className="card" aria-labelledby="configuration-transfer-title">
        ...
        <p className="hint">
          The versioned export contains mapping profiles, approved aliases and
          settings only. It excludes imported business rows, operational prices
        ...
        {platform.kind === "desktop" && migration?.legacyConfigurationFound && (
          <div
            className={migration.valid ? "callout" : "callout callout-danger"}
            role="status"
          >
            <strong>Legacy WebView migration inspection</strong>
            <p>
              Found {migration.counts.profiles} profile record(s),{" "}
              {migration.counts.aliases} alias record(s) and{" "}
              {migration.counts.settings} settings record. Invalid:{" "}
              {migration.invalidCounts.profiles} profiles,{" "}
              {migration.invalidCounts.aliases} aliases and{" "}
              {migration.invalidCounts.settings} settings record.
            </p>
            {migration.alreadyImported && (
              <p>
                This exact legacy configuration was already imported and
                verified.
              </p>
            )}
        ...
        {preview !== null && (
          <div className="callout callout-warn" role="status">
            ...
            <p>
              Incoming: {preview.counts.profiles} profiles,{" "}
              {preview.counts.aliases} aliases and {preview.counts.settings}{" "}
              settings record. Conflicts: {preview.conflicts.profiles} profiles,{" "}
              {preview.conflicts.aliases} aliases and{" "}
              {preview.conflicts.settings} settings record. Applying requires
              this separate confirmation and creates a verified backup first.
            </p>
