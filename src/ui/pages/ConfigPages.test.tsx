...
describe("backup and recovery UI", () => {
  it("lists and creates a manual verified backup", async () => {
    ...
    expect(
      await screen.findByText(/no verified backup is available/i),
    ).toBeInTheDocument();
    await user.click(
  ...
  it("requires a verified preview and separate acknowledgement before restoring", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /preview selected backup/i }),
    );
    expect(
      await screen.findByText(/verified restore preview/i),
    ).toBeInTheDocument();
    expect(calls).toContainEqual({
    ...
    await user.click(
      screen.getByRole("checkbox", { name: /restore replaces/i }),
    );
  ...
  it("keeps the workflow blocked when post-restore configuration reload fails", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /restore selected backup/i }),
    );

    expect(
      await screen.findByText(/workflow remains blocked/i),
    ).toBeInTheDocument();
    expect(announce).toHaveBeenCalledWith(
