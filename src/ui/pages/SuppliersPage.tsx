...
export function SuppliersPage() {
  ...
  const confirmDelete = async () => {
    ...
    await db.deleteProfile(pendingDelete.id);
    dispatch({
      type: 'profiles-loaded',
      profiles: state.profiles.filter((p) => p.id !== pendingDelete.id),
    });
  ...
  return (
    <Page title="Suppliers">
      ...
      <ConfirmDialog
        ...
        body={`Delete the saved mapping profile "${pendingDelete?.name ?? ''}" from this browser? This cannot be undone.`}
