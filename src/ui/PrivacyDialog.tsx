...
export function PrivacyDialog({ open, onClose }: PrivacyDialogProps) {
  ...
  const applyReset = async () => {
    if (!resetPreview || confirmation !== resetPreview.confirmationPhrase)
      return;
    setResetting(true);
    const result = await platform.recovery.reset(
      resetPreview.resetToken,
      confirmation,
    );
    setResetting(false);
    if (!result.ok) {
  ...
  return (
    <>
      ...
      >
        ...
        <ul className="small">
          <li>
            Supplier and ServiceM8 files are memory only and are never uploaded
          </li>
          <li>Raw imported rows are not written to the operational database</li>
          <li>
            Generated files stay local and leave application memory only when
            you explicitly save them through the native picker or download them
            from the web demonstration
          </li>
        </ul>
        ...
        {resetPreview !== null && (
          ...
          >
            ...
            <p>
              A verified backup is created first. Uninstall is not affected and
              never erases this data. This action removes only:
            </p>
            <ul>
            ...
            <p className="small">
              Records: {resetPreview.recordCounts.catalogueItems} catalogue
              items, {resetPreview.recordCounts.approvals} approvals,{" "}
              {resetPreview.recordCounts.priceHistory} price versions,{" "}
              {resetPreview.recordCounts.competitorReferences} references,{" "}
              {resetPreview.recordCounts.profiles} profiles and{" "}
              {resetPreview.recordCounts.aliases} aliases.
              {platform.kind === "desktop"
            ...
            <div className="btn-row">
              ...
              <button
                ...
                className="btn btn-danger"
                disabled={
                  confirmation !== resetPreview.confirmationPhrase || resetting
                }
                onClick={() => void applyReset()}
              >
                {resetting
                  ? "Creating backup and erasing…"
                  : "Create backup and erase exact scope"}
              </button>
            </div>
          </section>
        )}
