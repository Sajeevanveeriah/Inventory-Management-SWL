...
const allOutputs: GeneratedOutput[] = [
  output,
  ...(
    ["Change-Report", "Exceptions", "Rollback", "Audit-Summary"] as const
  ).map((name, index): GeneratedOutput => ({
    ...output,
...
describe("desktop export presentation", () => {
  it("does not expose browser download controls in the desktop application", () => {
    const { rerender } = render(
      <GeneratedFilesPanel outputs={[output]} platformKind="desktop" />,
    );
    expect(
  ...
  it("defaults a same-file conflict to cancel and offers another folder without overwrite", async () => {
    ...
    const invoke: InvokeFunction = async function invokeCommand<T>(
      command: string,
    ) {
      ...
      if (command === "reserve_export_batch") {
        throw Object.assign(
          new Error("An output with this name already exists."),
          {
            code: "conflict",
            retryable: false,
          },
