  ...
  const chooseNativeFile = async (role: FileRole) => {
    ...
    if (!selected.ok) {
      if (selected.error.code !== "cancelled")
        actions.announce(selected.error.message);
      return;
    }
  ...
  return (
    <div>
      <div className="card">
        ...
        <p className="muted">
          Load the untouched supplier export and the current ServiceM8 materials
          &amp; services export. Files stay in application memory and are never
          uploaded.
        </p>
      </div>
      ...
      <div className="btn-row" style={{ marginTop: "0.5rem" }}>
        ...
        {!bothLoaded && (
          <span className="muted small">
            Both files are required to continue.
          </span>
        )}
      </div>
    </div>
