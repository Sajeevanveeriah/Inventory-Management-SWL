...
export function SettingsDialog({ open, onClose }: SettingsDialogProps) {
  ...
  const markupValid = /^\d{1,3}(\.\d{1,2})?$/.test(markup);
  const changed = markup !== state.settings.markupPercent || tax !== state.settings.taxHandling;

  const apply = () => {
    const next: Settings = { ...state.settings, markupPercent: markup, taxHandling: tax };
    const parts: string[] = [];
    ...
    void actions.changeSettings(next, parts.join('; '));
    setConfirming(false);
    onClose();
  };
        ...
        <div>
          <div className="field">
            ...
            {!markupValid && (
              <span
                ...
                role="alert"
              >
                Enter a number between 0 and 999.99, e.g. 30.
              </span>
            )}
          </div>
          <fieldset>
            ...
            <p className="help small muted" style={{ margin: '0.2rem 0 0.5rem' }}>
              This tool does not infer or alter GST treatment. No transformation is applied under
              any option; the selection is recorded in the audit report so downstream use is
              unambiguous.
            </p>
          ...
          <div className="field">
            ...
            <select
              ...
              onChange={(e) => {
                void actions.changeSettings(
                  { ...state.settings, theme: e.target.value as Settings['theme'] },
