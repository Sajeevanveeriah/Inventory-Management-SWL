...
export type BrowserTestSearchScenario = 'results' | 'empty' | 'timeout' | 'provider_error';
...
function result(
  ...
  const priceAud = centsToAud(priceCents);
  return {
    searchQuery: query,
...
export function createBrowserTestSearchOutcome(
  ...
  const prices: [number, number, number] = [12_950 + offset, 14_250 + offset, 15_600 + offset];
  const results = [
  ...
  return {
    ...
    retrievedAt,
    cached: false,
    detail: 'Deterministic fictional browser test. No provider request or charge occurred.',
    coverage: {
      providerQueried: 'browser-test-fixture',
      ...
      comparableOffers: results.length,
      excludedOffers: 0,
    },
  };
}
