...
import { TAX_HANDLING_SHORT } from '../core/settings';
...
export function useActions() {
  ...
  useEffect(() => {
    for (const [guard, status] of approvalGuards.current) {
      ...
      if (status === 'committed' && state.review.committedApprovals[rowId] === true) {
  ...
  return useMemo(() => {
    ...
    const generateOutputs = async () => {
      if (state.configurationHydration.status !== 'ready') {
        announce('Export is blocked until stored configuration is loaded and verified.');
        return;
      }
      if (
      ...
      const outputs = await buildAllOutputs({
        ...
        taxHandling: TAX_HANDLING_SHORT[state.settings.taxHandling],
        newItemIncludesTaxes: state.comparison.newItemConvention.includesTaxes,
        newItemTaxRate: state.comparison.newItemConvention.taxRate,
    ...
    const approveRows = async (rowIds: string[]) => {
      if (state.configurationHydration.status !== 'ready') {
        announce('Approval is blocked until stored configuration is loaded and verified.');
        return false;
      }
      ...
      try {
        ...
        announce('Approval was not recorded because the local platform service failed safely.');
        return false;
      }
      if (!published.ok) {
        for (const guard of guards) approvalGuards.current.delete(guard);
    ...
    const attachReference = async (
      ...
      announce(`Reference price stored for ${itemId}. No cost or sell price changed.`);
      return true;
    };
