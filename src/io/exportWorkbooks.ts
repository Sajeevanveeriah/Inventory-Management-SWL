...
async function buildChangeReport(input: ExportInput): Promise<{ blob: Blob; sanitized: number }> {
  ...
  const rows = comparison.rows.map((row) => {
    ...
    return [
      ...
      writer.text(row.pricingProvenance?.productId ?? ''),
      writer.text(row.pricingProvenance?.itemKind ?? ''),
      writer.text(row.pricingProvenance?.brandName ?? ''),
      writer.text(row.pricingProvenance?.supplierName ?? ''),
      writer.text(row.pricingProvenance?.supplierSku ?? ''),
      writer.text(row.pricingProvenance?.offerId ?? ''),
      writer.text(row.pricingProvenance?.selectionMethod ?? ''),
      writer.text(row.pricingProvenance ? PRICE_BASIS_LABELS[row.pricingProvenance.costBasis] : ''),
      writer.text(row.pricingProvenance?.currency ?? ''),
      writer.text(row.pricingProvenance?.observedAt ?? ''),
      writer.text(row.pricingProvenance ? `${row.pricingProvenance.markupPercent}%` : ''),
      writer.text(row.pricingProvenance?.markupSource ?? ''),
      writer.text(row.pricingProvenance?.markupExplanation ?? ''),
