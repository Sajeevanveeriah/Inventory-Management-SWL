...
async function buildImportWorkbook(
  ...
  if (templateAdapted) {
    // Template adaptation: reuse the loaded ServiceM8 export's exact headers
    // and column order. Price changes re-emit the original row with only the
    // cost/sell columns replaced; new items fill only the mapped columns.
    headers = s8Table.headers;
    dataRows = importRows.map((row) => {
      const out: (string | number)[] = new Array<string | number>(headers.length).fill('');
      if (row.status === 'price-changed' && row.s8 !== null) {
        const original = s8Table.rows[row.s8.rowIndex] ?? [];
        for (let c = 0; c < headers.length; c += 1) out[c] = writer.text(original[c] ?? '');
      } else {
        if (numberCol !== undefined) out[numberCol] = writer.text(row.supplier?.code ?? '');
        if (descCol !== undefined) out[descCol] = writer.text(row.supplier?.description ?? '');
      }
  ...
  const ws = addTableSheet(wb, 'Import', headers, dataRows);
  ...
  addTableSheet(
    ...
    [
      ...
      ['Run identifier', ''],
      ['Approved price changes', importRows.filter((r) => r.status === 'price-changed').length],
      ['Approved new items', importRows.filter((r) => r.status === 'new-item').length],
...
async function buildChangeReport(input: ExportInput): Promise<{ blob: Blob; sanitized: number }> {
  ...
  const rows = comparison.rows.map((row) => {
    ...
    return [
      ...
      row.supplier?.cost != null && row.proposedSell !== null
        ? writer.text(markupFormula(row.supplier.cost, comparison.markupPercent))
        : '',
  ...
  addTableSheet(wb, 'All records', headers, rows);
  return { blob: await toBlob(wb), sanitized: writer.sanitized };
}
...
/** 4. Rollback copy — the original ServiceM8 rows, re-emitted verbatim. */
async function buildRollbackWorkbook(
  ...
  addTableSheet(
    ...
    ['Field', 'Value'],
    [
      ['Purpose', 'Exact copy of the ServiceM8 export as loaded, for rollback.'],
...
export async function buildAllOutputs(input: ExportInput): Promise<GeneratedOutput[]> {
  ...
  const name = (purpose: Parameters<typeof outputFilename>[2], ext: 'xlsx' | 'txt') =>
  ...
  const auditBlob = new Blob([buildAuditText(auditInput)], { type: 'text/plain;charset=utf-8' });
