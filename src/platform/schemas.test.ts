...
describe("LiveSearchResultSchema semantic boundary", () => {
  it("accepts an internally consistent direct merchant offer", () => {
    expect(LiveSearchResultSchema.safeParse(eligibleResult()).success).toBe(
      true,
    );
  });
  ...
  it.each([
    ...
    expect(
      LiveSearchResultSchema.safeParse(eligibleResult(provenance)).success,
    ).toBe(false);
  });
  ...
  it.each([
    ...
    expect(
      LiveSearchResultSchema.safeParse(
        eligibleResult({ [field]: invalidValue }),
      ).success,
    ).toBe(false);
  });
  ...
  it("rejects disagreement between the backwards-compatible and item-price aliases", () => {
    expect(
      LiveSearchResultSchema.safeParse(
        eligibleResult({ priceCents: 9_999, priceAud: "99.99" }),
      ).success,
    ).toBe(false);
  });
  ...
  it.each([
    ...
    expect(
      LiveSearchResultSchema.safeParse(eligibleResult(mutation)).success,
    ).toBe(false);
  });
  ...
  it.each([
    ...
    "rejects an ineligible offer marked comparison eligible: %o",
    (mutation) => {
      expect(
        LiveSearchResultSchema.safeParse(eligibleResult(mutation)).success,
      ).toBe(false);
  ...
  it("returns a validation failure instead of throwing for a malformed URL", () => {
    const parse = () =>
      LiveSearchResultSchema.safeParse(eligibleResult({ url: "not a URL" }));
...
describe("LiveSearchOutcomeSchema semantic boundary", () => {
  it("accepts an outcome whose band and coverage are derived from its results", () => {
    expect(LiveSearchOutcomeSchema.safeParse(validOutcome()).success).toBe(
      true,
    );
  });
  ...
  it.each([
    ...
    "rejects result provenance that disagrees in %s",
    (field, invalidValue) => {
      const outcome = validOutcome();
      outcome.results[0] = {
        ...outcome.results[0]!,
  ...
  it("requires complete provenance on every live result in a result outcome", () => {
    ...
    missingAllProvenance.results = missingAllProvenance.results.map(
      withoutSelectionProvenance,
    );
    expect(
      LiveSearchOutcomeSchema.safeParse(missingAllProvenance).success,
    ).toBe(false);

    const mixedProvenance = validOutcome();
    mixedProvenance.results[1] = withoutSelectionProvenance(
      mixedProvenance.results[1]!,
    );
    expect(LiveSearchOutcomeSchema.safeParse(mixedProvenance).success).toBe(
      false,
    );
  });
