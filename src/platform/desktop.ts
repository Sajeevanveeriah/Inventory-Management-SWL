...
const CONFIGURATION_EXPORT_FILENAME = /^\d{8}-[A-Za-z0-9][A-Za-z0-9_-]{0,198}\.json$/;
const ExportBatchReservationSchema = z.object({ batchId: z.string().uuid() }).strict();
...
async function invokeParsed<T>(
  ...
  try {
    ...
    return parsed.success
      ...
      : platformFail('integrity_failed', 'The desktop service returned an invalid response.');
  } catch (error) {
...
async function chooseAndReadInputFile(
  ...
  const maxBytes = role === 'configuration' ? MAX_CONFIGURATION_INPUT_BYTES : MAX_DATA_INPUT_BYTES;
  const expectedExtensions = role === 'configuration' ? ['json'] : ['csv', 'xlsx'];
  if (grant.length > maxBytes || !expectedExtensions.includes(grant.extension)) {
    await invokeVoid(invoke, 'release_input_grant', { grantId: grant.grantId });
    return platformFail('invalid_input', 'The selected input file is outside the supported range.');
  }
...
async function legacyConfigurationEnvelope(
  ...
  const bytes = new TextEncoder().encode(canonicalConfigurationPayload(withoutHash));
...
async function saveOneOutput(
  ...
  if (bytes.byteLength !== length) {
    return platformFail('integrity_failed', 'The output file changed during export preparation.');
  }
  const begin = await invokeParsed(invoke, 'begin_export_file', BeginExportSchema, {
    batchId,
    grantId,
  ...
  try {
    for (let offset = 0; offset < bytes.byteLength; offset += EXPORT_CHUNK_BYTES) {
      const chunk = bytes.subarray(offset, Math.min(offset + EXPORT_CHUNK_BYTES, bytes.byteLength));
      ...
      if (appended.value !== offset + chunk.byteLength) {
        await invokeVoid(invoke, 'abort_export_file', { sessionId });
        return platformFail('integrity_failed', 'The desktop export byte count did not match.');
      }
    }
    const committed = await invokeParsed(invoke, 'commit_export_file', z.string().min(1).max(512), {
      sessionId,
    });
    if (!committed.ok) await invokeVoid(invoke, 'abort_export_file', { sessionId });
    return committed;
  } catch (error) {
    await invokeVoid(invoke, 'abort_export_file', { sessionId });
    return errorResult(error);
...
export function createDesktopPlatformService(
  ...
  const service: PlatformService = {
    ...
    health: () =>
      ...
      >,
    catalogue: {
      list: () => invokeParsed(invoke, 'list_catalogue_items', z.array(CatalogueItemSchema)),
    ...
    priceHistory: {
      list: (itemId) =>
        invokeParsed(invoke, 'list_price_history', z.array(PriceHistoryVersionSchema), {
          itemId: itemId ?? null,
        }),
    },
    references: {
    ...
    sources: {
      async list() {
        const result = await invokeParsed(invoke, 'list_sources', z.array(CompetitorSourceSchema));
        if (!result.ok) return result;
        const sources = withoutLegacySyntheticSources(result.value);
        return platformOk(sources.length === 0 ? defaultSources() : sources);
      },
      replace: (sources) =>
        invokeParsed(invoke, 'replace_sources', z.array(CompetitorSourceSchema), { sources }),
    },
    profiles: {
      list: () => invokeParsed(invoke, 'list_mapping_profiles', z.array(MappingProfileSchema)),
      save: (profile) =>
    ...
    aliases: {
      list: () => invokeParsed(invoke, 'list_aliases', z.array(AliasRecordSchema)),
      save: (alias) => invokeParsed(invoke, 'save_alias', AliasRecordSchema, { alias }),
      delete: (supplierCode) => invokeVoid(invoke, 'delete_alias', { supplierCode }),
    },
    settings: {
      load: () =>
        invokeParsed(invoke, 'load_settings', z.unknown()).then((result) => {
          ...
          return parsed.success
            ? platformOk(parsed.data)
            : platformFail('integrity_failed', 'The stored settings are invalid.');
        }),
      save: async (settings) => {
    ...
    configuration: {
      ...
      async exportToSelectedFolder(filename) {
        if (!CONFIGURATION_EXPORT_FILENAME.test(filename)) {
          return platformFail('invalid_input', 'The configuration export filename is invalid.');
        }
        ...
        return invokeParsed(invoke, 'export_configuration_to_folder', z.string().max(260), {
      ...
      previewImport: (serialised) => {
        ...
        return byteLength === 0 || byteLength > MAX_CONFIGURATION_INPUT_BYTES
          ...
          : invokeParsed(invoke, 'preview_configuration_import', ConfigurationPreviewSchema, {
              serialised,
            });
      },
      ...
      async migrationStatus() {
        ...
        const legacySerialised =
          inspection.legacyConfigurationFound && inspection.valid && inspection.snapshot !== null
            ? JSON.stringify(await legacyConfigurationEnvelope(inspection.snapshot))
            : null;
        const native = await invokeParsed(
          invoke,
          'configuration_migration_status',
        ...
        return platformOk({
          ...
          counts: inspection.legacyConfigurationFound ? inspection.counts : native.value.counts,
        });
      },
      async previewLegacyImport() {
        ...
        if (
          ...
          JSON.stringify(envelope.data.settings) === JSON.stringify(DEFAULT_SETTINGS)
        ) {
          return platformFail('unavailable', 'No legacy WebView configuration was found.');
        }
    ...
    recovery: {
      ...
      listBackups: () => invokeParsed(invoke, 'list_backups', z.array(BackupSummarySchema)),
      ...
      previewReset: () => invokeParsed(invoke, 'preview_reset', ResetPreviewSchema),
    ...
    search: {
      status: () => invokeParsed(invoke, 'provider_status', ProviderStatusSchema),
      async query(query, candidateToken) {
        ...
        if (
          ...
          return {
            ...
            results: [],
            band: null,
            detail: 'Enter a product identifier or search phrase within the supported range.',
          };
        }
        const result = await invokeParsed(invoke, 'search_competitors', LiveSearchOutcomeSchema, {
          query,
      ...
      setPaidCallsEnabled: (enabled, costCeilingCents, costPerCallCents) => {
        ...
        return invokeParsed(invoke, 'set_provider_paid_calls', ProviderStatusSchema, {
          enabled,
      ...
      configureCredential: (secret) =>
        validCredential(secret)
          ? invokeParsed(invoke, 'configure_provider_credential', ProviderStatusSchema, { secret })
          : Promise.resolve(
              platformFail(
                'invalid_input',
      ...
      validateCredential: () =>
        invokeParsed(invoke, 'validate_provider_credential', ProviderStatusSchema),
      replaceCredential: (secret) =>
        validCredential(secret)
          ? invokeParsed(invoke, 'replace_provider_credential', ProviderStatusSchema, { secret })
          : Promise.resolve(
              platformFail(
                'invalid_input',
                'The provider credential is outside the supported range.',
      ...
      removeCredential: () =>
        invokeParsed(invoke, 'remove_provider_credential', ProviderStatusSchema),
    },
    files: {
      chooseInputFile: (role) => chooseAndReadInputFile(invoke, role),
      chooseOutputDestination: () =>
        invokeParsed(invoke, 'choose_output_destination', OutputDestinationGrantSchema.nullable()),
      async saveOutputs(destination, outputs) {
        ...
        for (const item of prepared) {
          const result = await saveOneOutput(invoke, batchId, destination.grantId, item);
          if (!result.ok) {
      ...
      openVerifiedSource: (url) => invokeVoid(invoke, 'open_verified_source', { url }),
    },
  };
