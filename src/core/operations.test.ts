...
async function demoComparison() {
  ...
  const s8 = await parseFile(
    new File([DEMO_SERVICEM8_CSV], "s8.csv", { type: "text/csv" }),
  );
  return runComparison(
    extractSupplierRecords(supplier, {
      supplierCode: 0,
...
describe("operations domain", () => {
  it("derives unified exceptions and proposal-only approvals", async () => {
    ...
    expect(
      deriveExceptions(comparison).some((e) => e.type === "ambiguous"),
    ).toBe(true);
    const proposals = buildApprovalProposals(comparison, {});
    expect(
      proposals.some((p) => !p.approvable && p.exceptionState === "invalid"),
    ).toBe(true);
    expect(proposals.some((p) => p.approvable && p.changeCount === 1)).toBe(
      true,
    );
  });
