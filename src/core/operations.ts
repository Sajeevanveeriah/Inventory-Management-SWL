  ...
  return comparison.rows
    ...
    .map((row) => {
      ...
      return {
        ...
        pricingRuleVersion: row.pricingProvenance?.ruleVersion ?? 'unresolved',
        oldValue: row.s8?.existingSell ?? '',
        proposedValue: row.proposedSell ?? '',
        markup: `${row.pricingProvenance?.markupPercent ?? comparison.markupPercent}% (${row.pricingProvenance?.markupSource ?? 'global'})`,
        ...
        changeSetHash: `${row.id}:${row.pricingProvenance?.offerId ?? 'unresolved'}:${row.pricingProvenance?.markupSource ?? 'unresolved'}:${row.proposedSell ?? 'blocked'}`,
        approvable,
      };
    });
}
