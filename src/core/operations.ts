  ...
  return comparison.rows
    ...
    .map((row) => {
      ...
      return {
        id: row.id,
        changeCount:
          row.status === "price-changed" || row.status === "new-item" ? 1 : 0,
        exceptionCount:
          row.status === "ambiguous" || row.status === "invalid" ? 1 : 0,
        ...
        reason:
          decision?.reason ?? row.messages[0]?.message ?? "Pending review",
...
export function buildRunMetadata(params: {
  ...
  return {
    ...
    localTimestamp: new Date().toISOString(),
    inputFilenames: params.inputFiles.map((file) => file.filename),
    fileHashes: params.inputFiles.map((file) => file.sha256),
    ...
    matchMethodTotals: Object.fromEntries(
      ["exact-code", "alias", "none"].map((method) => [
        method,
        params.comparison?.rows.filter((row) => row.matchMethod === method)
          .length ?? 0,
      ]),
    ),
    approvalTotals: {
      approved: Object.values(params.decisions).filter(
        (d) => d.state === "approved",
      ).length,
      excluded: Object.values(params.decisions).filter(
        (d) => d.state === "excluded",
      ).length,
      importEligible: importRows.length,
    },
    exceptionTotals: {
      blocking: deriveExceptions(params.comparison).filter(
        (e) => e.severity === "blocking",
      ).length,
      warning: deriveExceptions(params.comparison).filter(
    ...
    validationOutcome:
      importRows.length > 0
        ? "ready"
        : deriveExceptions(params.comparison).some(
              (e) => e.severity === "blocking",
            )
          ? "blocked"
          : "draft",
...
export function parseCompetitorEvidenceRows(
  ...
  rows.forEach((row, index) => {
    ...
    observations.push({
      ...
      gstBasis:
        (row.gstBasis as CompetitorObservation["gstBasis"]) || "unknown",
      shipping: row.shipping || "0",
      stockStatus:
        (row.stockStatus as CompetitorObservation["stockStatus"]) || "unknown",
      condition:
        (row.condition as CompetitorObservation["condition"]) || "unknown",
