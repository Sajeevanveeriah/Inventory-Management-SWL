...
import { AMBIGUITY_THRESHOLD, SUGGESTION_THRESHOLD, descriptionSimilarity } from './similarity';
...
export function runComparison(
  ...
  const duplicateSupplierCodes = new Set(
    [...supplierByCode.entries()].filter(([, v]) => v.length > 1).map(([k]) => k),
  );
  ...
  // --- alias sanity ---------------------------------------------------------
  // (Alias storage enforces one target per supplier code; a conflicting alias
  // would have to collide with an exact match, handled per-row below.)
  ...
  for (const sup of supplierRecords) {
    ...
    if (hasError) {
      rows.push(row(`sup-row:${sup.rowIndex}`, 'invalid', 'none', sup, null, null, null, messages));
      continue;
    }
    ...
    if (duplicateSupplierCodes.has(sup.codeNorm)) {
      ...
      rows.push(
        row(`sup-row:${sup.rowIndex}`, 'ambiguous', 'none', sup, null, null, null, messages),
      );
      continue;
    }
    ...
    if (targetNorm !== null) {
      if (duplicateS8Numbers.has(targetNorm)) {
        ...
        rows.push(row(`sup:${sup.codeNorm}`, 'ambiguous', method, sup, null, null, null, messages));
        continue;
      }
      ...
      if (s8.issues.some((i) => i.severity === 'error')) {
        ...
        rows.push(row(`sup:${sup.codeNorm}`, 'invalid', method, sup, s8, null, null, messages));
        continue;
      }
      ...
      if (amountEquals(cost, existingCost)) {
        rows.push(
          row(`sup:${sup.codeNorm}`, 'unchanged', method, sup, s8, proposed, delta, messages),
        );
      } else {
        rows.push(
          row(`sup:${sup.codeNorm}`, 'price-changed', method, sup, s8, proposed, delta, messages),
        );
      }
      continue;
    }
    ...
    for (const s8 of s8Records) {
      ...
      if (similarity >= SUGGESTION_THRESHOLD) {
        suggestions.push({ itemNumber: s8.itemNumber, description: s8.description, similarity });
      }
    }
    ...
    if (strongest !== undefined && strongest.similarity >= AMBIGUITY_THRESHOLD) {
      ...
      rows.push(
        row(`sup:${sup.codeNorm}`, 'ambiguous', 'none', sup, null, null, null, messages, top),
      );
      continue;
    }
    ...
    rows.push(
      row(`sup:${sup.codeNorm}`, 'new-item', 'none', sup, null, proposed, null, messages, top),
    );
  }
  ...
  for (const s8 of s8Records) {
    ...
    if (s8.issues.some((i) => i.severity === 'error')) {
      rows.push(row(`s8-row:${s8.rowIndex}`, 'invalid', 'none', null, s8, null, null, messages));
      continue;
    }
    if (duplicateS8Numbers.has(s8.itemNumberNorm)) {
      ...
      rows.push(row(`s8-row:${s8.rowIndex}`, 'ambiguous', 'none', null, s8, null, null, messages));
      continue;
    }
    messages.push({
      severity: 'info',
...
function row(
  ...
  return { id, status, matchMethod, supplier, s8, proposedSell, costDelta, messages, suggestions };
}
