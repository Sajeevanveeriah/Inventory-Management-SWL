...
describe("suggestMappings", () => {
  it("suggests likely columns from headers without auto-applying anything", () => {
    ...
    const byField = Object.fromEntries(
      suggestions.map((s) => [s.field, s.columnIndex]),
    );
  ...
  it("returns nothing for unrecognisable headers", () => {
    expect(
      suggestMappings(["Alpha", "Beta", "Gamma"], SUPPLIER_FIELDS),
    ).toHaveLength(0);
  });
});
...
describe("validateMapping", () => {
  ...
  it("detects the same column mapped to two fields", () => {
    ...
    expect(issues.some((i) => i.message.includes("more than one field"))).toBe(
      true,
    );
  });
  ...
  it("detects mappings to columns that no longer exist", () => {
    ...
    expect(issues.some((i) => i.message.includes("no longer exists"))).toBe(
      true,
    );
  });
});
