...
export type RecommendationStrategy =
  'MATCH' | 'UNDERCUT_AMOUNT' | 'UNDERCUT_PERCENT' | 'MAINTAIN_FLOOR';
...
export function normaliseObservationEx(observation: CompetitorObservation, gstRate = '0.10') {
  ...
  if (observation.gstBasis === 'inc-gst')
    return price.div(new Big(1).plus(gstRate)).round(2, Big.roundHalfUp);
...
export function recommendCompetitivePrice(params: {
  ...
  if (!eligible.length)
    return {
      exception: 'NO_VALID_OBSERVATION',
      floorEx: floor.toFixed(2),
      blocked: true,
      reason: 'No valid accepted competitor observation is eligible.',
    };
  const lowest = eligible.sort((a, b) => a.cmp(b))[0] as Big;
  const target =
    strategy === 'UNDERCUT_AMOUNT'
      ? lowest.minus(params.undercutAmount ?? '1').round(2, Big.roundHalfUp)
      : strategy === 'UNDERCUT_PERCENT'
        ? lowest.times(new Big(1).minus(params.undercutPercent ?? '0.05')).round(2, Big.roundHalfUp)
        : strategy === 'MAINTAIN_FLOOR'
          ? floor
          : lowest;
  const recommended = (floor.gt(target) ? floor : target).round(2, Big.roundHalfUp);
  const belowFloor = target.lt(floor);
  return {
    exception: belowFloor ? 'COMPETITOR_BELOW_FLOOR' : 'OK',
    normalisedCompetitorEx: lowest.toFixed(2),
    floorEx: floor.toFixed(2),
    targetEx: target.toFixed(2),
    recommendedEx: recommended.toFixed(2),
    blocked: belowFloor,
    reason: belowFloor
      ? 'Competitor target is below the configured cost floor.'
      : 'Recommendation passes the cost floor.',
  };
}
