...
function row(id: string, status: ComparisonRow['status'], category?: string): ComparisonRow {
  return {
    id,
    status,
    matchMethod: 'none',
...
describe('buildExpansionCatalogue', () => {
  it('groups only supplier-only items and keeps uncategorised records visible', () => {
    ...
    expect(categories.map((category) => category.name)).toEqual(['Safes', 'Uncategorised']);
    expect(categories[0]?.items.map((item) => item.code)).toEqual(['A-1', 'B-2']);
    expect(categories.every((category) => category.scope === 'out-of-scope')).toBe(true);
    expect(
      categories
