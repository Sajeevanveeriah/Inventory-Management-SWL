...
import { createPaidCallBudgetFromEnvironment, createSearchService } from './search/service.mjs';
 ...
 *   SWL_SEARCH_PROVIDER  serpapi, serper or ebay (auto-selects a configured no-app-budget provider when omitted)
...
const argValues = (name) =>
  args.flatMap((value, index) =>
    value === name && typeof args[index + 1] === 'string' ? [args[index + 1]] : [],
  );

const port = Number(argValue('--port') ?? process.env.PORT ?? 8787);
const dataDir = process.env.SWL_DATA_DIR ?? join(HERE, 'data');
if (args.includes('--fixture') || process.env.SWL_SEARCH_PROVIDER === 'fixture') {
  throw new Error('Fixture search is unavailable from the production server entry point.');
}
const testProviderFactory = globalThis.__SWL_TEST_ONLY_SEARCH_PROVIDER_FACTORY__;
delete globalThis.__SWL_TEST_ONLY_SEARCH_PROVIDER_FACTORY__;
const fixtureMode = typeof testProviderFactory === 'function';
const provider = fixtureMode ? testProviderFactory() : createProviderFromEnvironment(process.env);
...
for (const candidate of argValues('--trusted-origin')) {
  ...
  if (
    ...
    throw new Error('Trusted development origins must be canonical loopback HTTP origins.');
  }
...
function requestBoundaryFailure(req, url) {
  ...
  const exactOrigin = typeof origin === 'string' && TRUSTED_WEB_ORIGINS.has(origin);
...
async function handleApi(req, res, url) {
  ...
  if (route === 'GET /api/health') {
    ...
    return sendJson(res, 200, {
      ...
      paidCallsEnabled:
        !fixtureMode && provider.requiresPaidCall === true && paidPolicy.state === 'enabled',
  ...
  if (route === 'GET /api/providers') {
    const providers = [provider, ...optionalProviderRegistry()].filter(
      (candidate, index, all) => all.findIndex((entry) => entry.name === candidate.name) === index,
    );
  ...
  if (route === 'POST /api/competitor-search') {
    ...
    const expected = body?.candidateToken === undefined ? ['query'] : ['candidateToken', 'query'];
    if (
  ...
  if (route === 'POST /api/publish-approved-changes') {
    ...
    try {
      ...
      if (error instanceof FloorViolationError || error instanceof PublicationValidationError) {
