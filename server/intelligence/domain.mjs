...
export function normaliseGtin(value) {
  ...
  if (!validLength)
    return { raw, normalised: digits, valid: false, reason: 'INVALID_LENGTH_OR_CHARACTERS' };
...
export function scoreProductMatch(item, candidate) {
  ...
  if (leftGtin.valid && rightGtin.valid) {
    ...
    } else return { score: 0, classification: 'rejected', reasons: ['GTIN_CONFLICT'] };
  }
  ...
  if (item.packQuantity && candidate.packQuantity && item.packQuantity !== candidate.packQuantity) {
    return { score: 0, classification: 'rejected', reasons: [...reasons, 'PACK_MISMATCH'] };
  }
  if (
    item.variant &&
    candidate.variant &&
    ...
    return { score: 0, classification: 'rejected', reasons: [...reasons, 'VARIANT_MISMATCH'] };
  }
...
export function normaliseOffer(
  ...
  return { ...offer, match, landedUnitCents, eligible: exclusions.length === 0, exclusions };
}
...
export function recommendPrice({ costCents, currentSellCents, offers, policy = DEFAULT_POLICY }) {
  if (!Number.isInteger(costCents))
    return { state: 'MANUAL_REVIEW_REQUIRED', reason: 'MISSING_COST', recommendationCents: null };
  const floorCents = Math.ceil((costCents * (10000 + policy.floorMarkupBasisPoints)) / 10000);
  ...
  if (sellers < policy.minimumSellersForManualReview)
    return { state: 'EVIDENCE_ONLY', reason: 'ONE_SELLER', ...common, recommendationCents: null };
