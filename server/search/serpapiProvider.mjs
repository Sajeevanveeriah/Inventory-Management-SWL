...
function componentToCents(extracted, text, options) {
  ...
  if (hasExtracted && hasText) {
    return validCents(extractedCents) && validCents(textCents) && extractedCents === textCents
      ? extractedCents
      : COMPONENT_CONFLICT;
  }
...
async function readBoundedJson(response) {
  ...
  if (declaredLength !== null) {
    if (!/^\d+$/u.test(declaredLength) || Number(declaredLength) > MAX_PROVIDER_RESPONSE_BYTES) {
...
function providerTimestamp(value) {
  if (!boundedProviderText(value, 64)) return null;
  const match = /^(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2}) UTC$/u.exec(value);
...
function providerMeta(body, storesMayContinue) {
  ...
  const observedAt =
    providerTimestamp(metadata.processed_at) ?? providerTimestamp(metadata.created_at);
  return {
    observedAt,
    cacheBasis:
...
function normaliseCandidate(item) {
  ...
  if (!boundedProviderText(item.immersive_product_page_token, MAX_TOKEN_CHARACTERS)) {
    return null;
  }
  const productUrl = googleProductUrl(item.product_link);
  if (!productUrl) return null;
  if (!Number.isSafeInteger(item.position) || item.position < 0 || item.position > 10_000) {
    return null;
  }
  ...
  return {
    ...
    productId: boundedProviderText(item.product_id, 256) ? item.product_id : null,
    productUrl,
...
function shoppingRows(body) {
  ...
  for (const category of checkedArray(body, 'categorized_shopping_results')) {
    if (!category || typeof category !== 'object' || Array.isArray(category)) {
      throw new ProviderRequestError('categorized shopping results are invalid');
    }
...
function validateDiscoveryParameters(body, expectedLocation, expectedQuery) {
  ...
  for (const [key, expected] of [
    ...
    if (key in parameters && parameters[key] !== expected) {
      throw new ProviderRequestError('shopping response localisation is invalid');
    }
  }
  if (
    'q' in parameters &&
    canonicalProviderQuery(parameters.q) !== canonicalProviderQuery(expectedQuery)
  ) {
...
function boundedDetails(store) {
  ...
  if (!Array.isArray(store.details_and_offers) || store.details_and_offers.length > 50) {
    return null;
  }
  const details = store.details_and_offers.filter((entry) => boundedProviderText(entry, 512));
...
function offerAvailability(store, details) {
  ...
  return /\b(?:in stock|available online)\b/iu.test(combined) ? 'in-stock' : 'unknown';
}
...
function normaliseOffer(store, selectedCandidate) {
  ...
  if (!boundedProviderText(store.name, 512) || !boundedProviderText(store.title, 1_000)) {
    return null;
  }
  const merchantUrl = directMerchantUrl(store.link);
  if (!merchantUrl) return null;
  const originalPriceText = boundedProviderText(store.price, 64) ? store.price : null;
  ...
  const shippingCents = componentToCents(store.shipping_extracted, store.shipping, {
    allowFree: true,
  });
  const estimatedTaxCents = componentToCents(store.extracted_estimated_tax, store.estimated_tax);
...
function offersPayload(body, selectedCandidate, expectedToken) {
  ...
  if (
    ...
    throw new ProviderRequestError('immersive product response parameters are invalid');
  }
  ...
  if (nextPageToken !== undefined && !boundedProviderText(nextPageToken, MAX_TOKEN_CHARACTERS)) {
  ...
  for (const store of stores) {
    ...
    if (existingIndex === undefined) {
      ...
      offers.push(offer);
    } else if (offers[existingIndex].comparisonEligible && !offer.comparisonEligible) {
...
async function requestProvider(fetchImpl, params, signal) {
  ...
  if (
    ...
    throw new ProviderRequestError('provider endpoint is outside the allowlist');
  }
  ...
  if (response.url) {
    ...
    if (!responseUrl || responseUrl.hostname !== ALLOWED_PROVIDER_HOST) {
      throw new ProviderRequestError('provider response escaped the host allowlist');
    }
  }
...
export function createSerpApiProvider(env = process.env, fetchImpl = fetch) {
  ...
  function resolveCandidateSelection(providerQuery, { candidateToken, selectedCandidate } = {}) {
  ...
  return {
    ...
    async search(providerQuery, { signal, candidateToken, selectedCandidate } = {}) {
      if (apiKey === '') {
