...
function serperShippingCents(item) {
  const numeric = amountToCents(item.shippingCost ?? item.shipping_cost ?? item.extracted_shipping);
...
function serperOffer(item) {
  ...
  const itemPriceCents = amountToCents(item.extractedPrice ?? item.extracted_price ?? item.price);
  if (!title || !sourceUrl || itemPriceCents === null) return null;
  const seller = boundedProviderText(item.source, 512) ? item.source : sourceUrl.hostname;
  ...
  return createStructuredOffer({
    ...
    availabilityText: boundedProviderText(item.availability, 512, true) ? item.availability : '',
    financing: item.installment !== undefined || item.installments !== undefined,
  });
}

export function createSerperShoppingProvider(env = process.env, fetchImpl = fetch) {
