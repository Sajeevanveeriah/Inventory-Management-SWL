...
function validatedHttpsUrl(value, sourceDomain) {
  ...
  if (
    ...
    parsed.username !== '' ||
    parsed.password !== '' ||
    (sourceDomain !== undefined && parsed.hostname.toLowerCase() !== sourceDomain.toLowerCase())
  ) {
    throw new ProviderRequestError('provider result URL is outside the approved boundary');
  }
  return parsed;
}
...
function validateProviderResults(value) {
  if (!Array.isArray(value) || value.length > MAX_PROVIDER_RESULTS) {
    throw new ProviderRequestError('provider result count is outside the supported range');
  }
  const expectedKeys = [
    'gstBasis',
    'packSize',
  ...
  return value.map((item) => {
    ...
    if (
      ...
      (item.packSize !== null && !isBoundedProviderText(item.packSize, 256, true)) ||
      !isBoundedProviderText(item.seller, 512) ||
      !isBoundedProviderText(item.sourceDomain, 253)
    ) {
      throw new ProviderRequestError('provider result is outside the supported range');
    }
...
function validateProviderMeta(value) {
  exactKeys(value, ['cacheBasis', 'observedAt', 'storesMayContinue'], 'provider metadata');
  if (
    !['provider_cache_allowed', 'provider_cache_bypassed'].includes(value.cacheBasis) ||
...
function validateCandidate(value) {
  ...
  if (
    ...
    (value.displayedPrice !== null && !isBoundedProviderText(value.displayedPrice, 64)) ||
    !validNullableCents(value.priceCents) ||
    typeof value.multipleSources !== 'boolean' ||
    (value.packSize !== null && !isBoundedProviderText(value.packSize, 256, true)) ||
...
function validateOffer(value) {
  ...
  if (
    ...
    !['provider_total', 'item_plus_shipping', 'not_comparable'].includes(value.priceBasis) ||
    !isBoundedProviderText(value.originalPriceText, 64) ||
    !['explicit-aud', 'inferred-au-localisation'].includes(value.currencyBasis) ||
    ...
    value.exclusionReasons.some((reason) => !isBoundedProviderText(reason, 128)) ||
    ...
    if (
      ...
      (value.priceBasis === 'item_plus_shipping' &&
        (!validCents(value.shippingCents) ||
          value.comparisonPriceCents !== value.itemPriceCents + value.shippingCents))
    ) {
  ...
  if (
    (value.financing && value.totalPriceCents === null && value.comparisonEligible) ||
...
function validateStructuredProviderPayload(value) {
  ...
  if (value.stage === 'discovery') {
    exactKeys(value, ['candidates', 'providerMeta', 'stage'], 'provider discovery payload');
    if (!Array.isArray(value.candidates) || value.candidates.length > MAX_PROVIDER_RESULTS) {
      throw new ProviderRequestError('provider candidate count is outside the supported range');
    }
    return {
      stage: 'discovery',
  ...
  if (value.stage === 'offers') {
    ...
    if (!Array.isArray(value.offers) || value.offers.length > MAX_PROVIDER_RESULTS) {
      throw new ProviderRequestError('provider offer count is outside the supported range');
    }
    return {
      stage: 'offers',
...
export function createPaidCallBudgetFromEnvironment(env = {}) {
  ...
  const ceilingCents = parsePositiveInteger(env.SWL_PROVIDER_COST_CEILING_CENTS);
  const perCallCents = parsePositiveInteger(env.SWL_PROVIDER_COST_PER_CALL_CENTS);
  const explicitlyDisabled = flag === undefined || flag === '' || flag === 'false';
  const configurationValid =
    explicitlyDisabled || (flag === 'true' && ceilingCents !== null && perCallCents !== null);
...
function providerDetail(providerMeta, prefix = '') {
  ...
  if (providerMeta.storesMayContinue) {
    notes.push('The provider reports additional offers beyond this bounded response.');
  }
  if (providerMeta.cacheBasis === 'provider_cache_allowed') {
    notes.push('The provider may serve an identical request from its own cache.');
  }
  return notes.join(' ');
}
...
function offersOutcome(base, payload, retrievedAt) {
  const results = payload.offers.map((item) => ({
    ...
    shippingAud: item.shippingCents === null ? null : centsToAmount(item.shippingCents),
    estimatedTaxCents: item.estimatedTaxCents,
    estimatedTaxAud: item.estimatedTaxCents === null ? null : centsToAmount(item.estimatedTaxCents),
    totalPriceCents: item.totalPriceCents,
    totalPriceAud: item.totalPriceCents === null ? null : centsToAmount(item.totalPriceCents),
    comparisonPriceCents: item.comparisonPriceCents,
    comparisonPriceAud:
      item.comparisonPriceCents === null ? null : centsToAmount(item.comparisonPriceCents),
  ...
  return {
    ...
    async search(rawQuery, rawCandidateToken, rawSelectedCandidate) {
      ...
      const candidateToken =
        rawCandidateToken === undefined || rawCandidateToken === '' ? null : rawCandidateToken;
      const invalidQuery =
        kind === 'empty' ||
      ...
      const invalidCandidateToken =
        candidateToken !== null && !isBoundedProviderText(candidateToken, MAX_TOKEN_CHARACTERS);
      let selectedCandidate;
      try {
        if (rawSelectedCandidate !== undefined) {
          if (candidateToken === null) {
            throw new ProviderRequestError('selected candidate requires a candidate token');
          }
          selectedCandidate = validateCandidate(rawSelectedCandidate);
          if (selectedCandidate.token !== candidateToken) {
            throw new ProviderRequestError('selected candidate token does not match');
          }
        }
      } catch {
      ...
      if (candidateToken !== null && typeof provider.resolveCandidateSelection === 'function') {
        try {
          selectedCandidate = provider.resolveCandidateSelection(providerQuery, {
            candidateToken,
