...
import { matchServiceM8Layout, SERVICEM8_COLUMNS } from '../src/core/servicem8Format';
...
function mappingFrom(headers: string[], wanted: Record<string, string>): ColumnMapping {
...
describe.skipIf(!enabled)('verification against real supplier and ServiceM8 exports', () => {
  it('produces an import file in ServiceM8’s exact format with correct GST handling', async () => {
    ...
    const supplierRecords = extractSupplierRecords(supplierTable, supplierMapping);
    ...
    const csv = await (importOutput as NonNullable<typeof importOutput>).blob.text();
    ...
    for (const row of rows) expect(row).toHaveLength(SERVICEM8_COLUMNS.length);
    ...
    for (const row of rows.slice(1)) {
      ...
      expect(Number(row[priceCol])).toBeGreaterThanOrEqual(Number(row[costCol]));
    }
    ...
    const rollbackCsv = await (outputs[3] as NonNullable<(typeof outputs)[3]>).blob.text();
