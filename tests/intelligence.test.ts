...
describe('identity normalisation and explainable matching', () => {
  ...
  it('classifies exact, ambiguous and rejected cases with reasons', () => {
    ...
    expect(scoreProductMatch(item, { title: item.title }).classification).toBe('ambiguous');
    expect(scoreProductMatch(item, raw({ packQuantity: 2 }))).toMatchObject({
      classification: 'rejected',
      reasons: expect.arrayContaining(['PACK_MISMATCH']),
    });
    expect(scoreProductMatch(item, raw({ variant: 'polished brass' }))).toMatchObject({
      classification: 'rejected',
...
describe('offer comparison safety', () => {
  it.each([
    ...
    expect(
      normaliseOffer(raw(change), item, { now: '2026-08-09T00:00:00.000Z' }).exclusions,
    ).toContain(reason);
  });
  it('normalises known GST and shipping to landed AUD per unit', () => {
    ...
    expect(
      normaliseOffer(raw({ amountCents: 10000, shippingCents: 1000, gstStatus: 'ex-gst' }), item, {
        now: '2026-08-09T00:00:00.000Z',
      }).landedUnitCents,
    ).toBe(12100);
  });
  it('deduplicates a seller-price pair and flags a robust high outlier', () => {
    const offers = [10000, 10000, 10100, 10200, 50000].map((landedUnitCents, i) => ({
    ...
    expect(
      result.find((x: { landedUnitCents: number }) => x.landedUnitCents === 50000)?.outlier,
    ).toBe(true);
  });
});
...
describe('versioned pricing policy', () => {
  it('uses integer half-up markup with parity to the canonical 30 percent floor', () => {
    for (const costCents of [0, 1, 2, 5, 9, 10, 99, 999, 10_000, 123_456_789]) {
      expect(applyMarkupBasisPointsHalfUp(costCents, 3_000)).toBe(minimumSellPriceCents(costCents));
    }
  ...
  it('returns no recommendation when the floor prevents a strict undercut', () => {
    expect(recommendPrice({ costCents: 7700, currentSellCents: 11000, offers })).toMatchObject({
      state: 'NO_SAFE_UNDERCUT',
      recommendationCents: null,
    });
  });
  it('gates one and two seller evidence', () => {
    expect(recommendPrice({ costCents: 5000, offers: offers.slice(0, 1) }).state).toBe(
      'EVIDENCE_ONLY',
    );
    expect(recommendPrice({ costCents: 5000, offers: offers.slice(0, 2) }).state).toBe(
