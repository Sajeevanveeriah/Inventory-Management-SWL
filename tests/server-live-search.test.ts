...
describe("SerpAPI transport and response boundary", () => {
      ...
      candidates: [
        {
          ...
          productUrl:
            "https://www.google.com.au/shopping/product/product-main?gl=au",
        ...
        },
        {
          ...
          productUrl:
            "https://www.google.com.au/shopping/product/product-used?gl=au",
  ...
  it("uses the default Geelong locality and then fetches direct immersive store offers", async () => {
    ...
    expect(discoveryRequest.searchParams.get("location")).toBe(
      "Geelong, Victoria, Australia",
    );
    expect(discoveryRequest.searchParams.get("device")).toBe("desktop");
    expect(offersRequest.searchParams.get("engine")).toBe(
    ...
    expect(selected.offers[2]).toMatchObject({
      ...
      exclusionReasons: [
        "financing_without_full_total",
        "unknown_comparison_total",
      ],
    });
  ...
  it("rejects arbitrary, cross-query and refreshed candidate tokens before transport", async () => {
    ...
    discoveryBody = JSON.parse(
      JSON.stringify(shoppingDiscoveryFixture).replaceAll(
        "token-main",
        "replacement-token",
      ),
    ) as Record<string, unknown>;
  ...
  it("excludes an explicit merchant multi-pack when the selected cluster has no pack claim", async () => {
    ...
    const stores = (body.product_results as Record<string, unknown>)
  ...
  it("drops offers when numeric and displayed component evidence disagrees", async () => {
    ...
    const baseStore = structuredClone(
      (product.stores as Array<Record<string, unknown>>)[0]!,
    );
    product.stores = [
  ...
  it.each([
    ...
    [
      "wrong token",
      { engine: "google_immersive_product", page_token: "token-other" },
    ],
  ...
  it("deterministically drops malformed provider items before the shared result boundary", async () => {
    ...
    const shoppingResults = malformed.shopping_results as Array<
...
describe("two-stage comparison service contract", () => {
  function twoStageService(immersiveBody = immersiveOffersFixture) {
    const provider = createSerpApiProvider(
      ...
      async (url: string | URL | Request) => {
        ...
        const body = structuredClone(shoppingDiscoveryFixture);
        (body.search_parameters as Record<string, unknown>).q =
  ...
  it('returns "no_comparable_offers" when every direct offer is excluded', async () => {
    ...
    const productResults = noComparable.product_results as Record<
      string,
      unknown
    >;
    productResults.stores = (
      productResults.stores as Array<Record<string, unknown>>
    ).filter((store) =>
      [
        "Finance Merchant",
        "Used Merchant",
        "Unknown Delivery Merchant",
      ].includes(String(store.name)),
    );
  ...
  it("rejects an oversized candidate token before any provider call", async () => {
    ...
    expect(
      await service.search("Lockwood 001", "x".repeat(8_193)),
    ).toMatchObject({ state: "invalid_query", candidates: [], results: [] });
    expect(calls).toBe(0);
  });
  ...
  it("returns selection_expired before rate, budget, cache or transport work", async () => {
    ...
    expect(
      await service.search("Different product", "token-main"),
    ).toMatchObject({
