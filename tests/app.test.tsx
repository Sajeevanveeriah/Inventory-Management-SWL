...
function publicationService(
  succeeds: boolean,
  observe?: (
    changes: Parameters<PlatformService["catalogue"]["publishApproved"]>[0],
  ) => void,
): PlatformService {
...
describe("application workflow (jsdom integration)", () => {
    ...
    const service: PlatformService = {
      ...
      settings: {
        ...
        async load() {
          return platformFail(
            "integrity_failed",
            "Synthetic stored settings failed validation.",
          );
        },
      },
    };
  ...
  it("does not expose browser-download controls on desktop routes", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /privacy and data handling/i }),
    );
    await user.click(
      screen.getByRole("button", {
  ...
  it("describes Static Pages as session-only and provider-free", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: /run test search/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("searchbox", { name: /product name/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("combobox", { name: /test outcome/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByText(/Static Pages is provider-free and session-only/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("renders only platform API results when live competitor search is available", async () => {
    ...
    await user.type(
      screen.getByRole("searchbox", { name: /product name/i }),
      "Lockwood 4570",
    );
    await user.click(submit);

    expect(
      await screen.findByRole("region", { name: /live search results/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Test-only live adapter seller"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("button", { name: /attach as reference/i }),
    ).toBeDisabled();
  });
  ...
  it("does not report desktop live search ready when the next call exceeds its budget", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /^Competitor search$/ }),
    );
    expect(
      await screen.findByText(/live search budget is exhausted/i),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("searchbox", { name: /product name/i }),
    ).not.toBeInTheDocument();
  ...
  it("requires exact product selection before requesting merchant offers", async () => {
    ...
    await renderApp(service);
    await user.type(
      await screen.findByRole("searchbox", { name: /product name/i }),
      "LW4570",
    );
    await user.click(
      screen.getByRole("button", { name: /search live prices/i }),
    );
    ...
    await user.click(
      screen.getByRole("button", { name: /compare this exact product/i }),
    );

    expect(await screen.findByText("Observed merchant")).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: /selected exact product/i }),
    ).toBeInTheDocument();
  ...
  it("tells the operator to repeat discovery when a product selection expires", async () => {
    ...
    expect(
      screen.getByText(/run the product search again/i),
    ).toBeInTheDocument();
  });
  ...
  it("does not expose operator search controls for a fixture-mode service", async () => {
    ...
    const service: PlatformService = {
      ...
      search: {
        ...
        async query() {
          throw new Error(
            "Fixture-mode search must remain unreachable from operator UI.",
          );
        },
      },
    };
  ...
  it("does not expose an unpersisted business rule while a delayed settings save fails", async () => {
    ...
    let published: Parameters<
      PlatformService["catalogue"]["publishApproved"]
    >[0] = [];
    ...
    expect(
      screen.getByRole("button", { name: /saving verified settings/i }),
    ).toBeDisabled();

    finishSave(
      platformFail(
        "integrity_failed",
        "Synthetic settings write failed safely.",
      ),
    );
    expect(
      await screen.findAllByText(/synthetic settings write failed safely/i),
    ).not.toHaveLength(0);
    expect(
      screen.getByRole("button", { name: /confirm and apply/i }),
    ).toBeEnabled();
    ...
    await user.click(
      screen.getByRole("button", { name: /load synthetic demonstration/i }),
    );
    await screen.findByText("DEMO-fictionville-supplier-price-list.csv");
    await user.click(
      screen.getByRole("button", { name: /continue to column mapping/i }),
    );
    await user.click(
      screen.getByRole("button", {
    ...
    await user.click(
      screen.getByRole("button", { name: /review proposed changes/i }),
    );
    await user.click(
      screen.getByRole("button", { name: /^Price changed \(\d+\)$/ }),
    );
  ...
  it("walks the synthetic demo from start to the review workspace", async () => {
    ...
    expect(
      screen.getByRole("img", { name: "Stan Wootton Locksmiths" }),
    ).toBeInTheDocument();
    expect(screen.getAllByRole("heading", { level: 1 })).toHaveLength(1);
    expect(
      screen.getAllByText(/local processing only/i).length,
    ).toBeGreaterThan(0);
    ...
    await waitFor(() => {
      expect(
        screen.getByText("DEMO-fictionville-supplier-price-list.csv"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("DEMO-fictionville-servicem8-export.csv"),
      ).toBeInTheDocument();
    });
    ...
    expect(
      within(pipeline).getByText("Supplier records").previousSibling,
    ).toHaveTextContent("16");
    expect(
      within(pipeline).getByText("ServiceM8 records").previousSibling,
    ...
    await user.click(
      screen.getByRole("button", { name: /^Ambiguous \(\d+\)$/ }),
    );
    expect(screen.queryAllByRole("button", { name: /^Approve$/ })).toHaveLength(
      0,
    );
    ...
    expect(
      screen.getByRole("button", { name: /clear exclusion/i }),
    ).toBeDisabled();

    await user.click(screen.getByRole("button", { name: /^Approvals$/ }));
    expect(
      await screen.findByText(/recorded, append-only/i),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /withdraw approval/i }),
    ).not.toBeInTheDocument();
  }, 30_000);
  ...
  it("does not apply a web approval when atomic publication fails", async () => {
    ...
    expect(
      await screen.findAllByText(
        /approval was not recorded: synthetic publication failed/i,
      ),
    ).not.toHaveLength(0);
  ...
  it("records the operator-selected GST basis without transforming prices", async () => {
    ...
    >[0] = [];
    await renderApp(
      publicationService(true, (changes) => (published = changes)),
    );
  ...
  it("shows a specific rejection for unsupported files", async () => {
    ...
    await renderApp();
    await user.click(
      screen.getByRole("button", { name: /start new comparison/i }),
    );
    ...
    expect(
      await screen.findAllByText(/not a supported file type/i),
    ).not.toHaveLength(0);
    expect(screen.getByRole("alert")).toHaveTextContent(
      /not a supported file type/i,
    );
    expect(
      screen.getByText(/Only .csv and .xlsx files are accepted/i),
    ).toBeInTheDocument();
  });
  ...
  it("keeps export gated until the checklist passes", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /pre-export checks/i }),
    );
    ...
    expect(
      screen.getByRole("button", { name: /continue to export/i }),
    ).toBeDisabled();
  }, 30_000);
  ...
  it("persists manual evidence as a typed reference for an exact catalogue item", async () => {
    ...
    const base = publicationService(true);
    let attached:
      Parameters<PlatformService["references"]["attach"]>[1] | null = null;
    const service: PlatformService = {
      ...base,
      catalogue: {
        ...base.catalogue,
    ...
    await user.type(
      screen.getByRole("textbox", { name: /sku or product/i }),
      "LW4570",
    );
    await user.type(
      screen.getByRole("textbox", { name: /observed price/i }),
      "95.00",
    );
    await user.type(
      screen.getByRole("textbox", { name: /observed shipping/i }),
      "0",
    );
    await user.type(
    ...
    await user.click(
      screen.getByRole("button", { name: /store observation/i }),
    );
    ...
    expect(
      screen.getAllByText(/reference price stored for LW4570/i).length,
    ).toBeGreaterThan(0);
  });
  ...
  it("forwards the operator-entered provider budget through the page adapter boundary", async () => {
    ...
    let enabledArguments:
      [boolean, number | undefined, number | undefined] | null = null;
    const service: PlatformService = {
      ...base,
