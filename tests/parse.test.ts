...
describe('CSV parsing', () => {
  it('parses headers and rows, preserving leading zeroes and punctuation', async () => {
    const table = await parseFile(
      csvFile('demo.csv', 'Code,Name,Cost\n00123,"Fictional, thing",7.25\n=EVIL(),Injected,1.00\n'),
    );
  ...
  it('rejects files with too many columns', async () => {
    ...
    await expect(parseFile(csvFile('wide.csv', `${headers}\n${headers}\n`))).rejects.toThrow(
      /too many columns/,
    );
  });
  ...
  it('accepts the documented maximum row and column dimensions', async () => {
    ...
    expect(new TextEncoder().encode(content).byteLength).toBeLessThanOrEqual(MAX_FILE_BYTES);
...
describe('XLSX parsing', () => {
  ...
  it('rejects corrupted xlsx content', async () => {
    const junk = new File([new TextEncoder().encode('not really a zip')], 'fake.xlsx');
    await expect(parseFile(junk)).rejects.toThrow(/could not be read as an XLSX workbook/);
  });
});
