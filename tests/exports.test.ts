...
import { DEMO_SERVICEM8_CSV, DEMO_SUPPLIER_CSV, DEMO_ALIAS } from '../src/demo/fixtures';

const SUPPLIER_MAPPING = { supplierCode: 0, supplierDescription: 1, supplierCost: 2 };
const S8_MAPPING = { itemNumber: 0, itemDescription: 1, existingCost: 2, existingSellPrice: 3 };
...
describe('demo comparison end-to-end statuses', () => {
  it('assigns every expected demo status', async () => {
    ...
    const dupRows = comparison.rows.filter((r) => r.supplier?.code === 'FIC-005');
...
describe('buildAllOutputs', () => {
  it('generates five outputs whose import workbook contains only approved valid rows', async () => {
    ...
    const approvable = comparison.rows.filter(
      (r) => r.status === 'price-changed' || r.status === 'new-item',
    );
    let review = approveRows(EMPTY_REVIEW, approvable).state;
    const fic012 = comparison.rows.find((r) => r.supplier?.code === 'FIC-012');
    review = excludeRows(review, [fic012!], 'Fictional exclusion for testing').state;
