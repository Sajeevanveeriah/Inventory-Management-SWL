...
test("start screen has no serious accessibility violations", async ({
  ...
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: /supplier price comparison/i }),
  ).toBeVisible();
...
test("mapping, validation, review, checklist and export screens", async ({
  page,
}) => {
...
test("competitor search: empty, live results, no-results, provider failure; sources", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "Competitor search" }),
  ).toBeVisible();
...
test("dashboard, catalogue search and approvals with demo data", async ({
  page,
}) => {
  await demoToValidate(page);
