...
async function demoToValidate(page: Page) {
  ...
  await page
    .getByRole("radio", { name: /supplier costs exclude gst/i })
    .check();
...
test.describe("desktop 1440px light", () => {
  ...
  test("validate, review, checklist, export states", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: "Generate all output files" })
      .click();
    await expect(
  ...
  test("dialogs and dark theme", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: "Privacy and data handling" })
      .click();
...
test.describe("operations shell", () => {
  ...
  test("dashboard, search, exceptions, approvals, integrations", async ({
    page,
  }) => {
    ...
    await page
      .getByLabel("Search products by code, item number or description")
      .fill("deadbolt");
  ...
  test("competitor search: all five live states and source registry", async ({
    page,
  }) => {
    ...
    await page.getByRole("button", { name: "Search live prices" }).click();
    await page
      .getByRole("button", { name: "Compare this exact product" })
      .click();
    await expect(
      page.getByRole("region", { name: "Live search results" }),
    ).toBeVisible();
    ...
    await page.getByRole("button", { name: "Search live prices" }).click();
    await expect(
      page.getByText(/No usable product candidates found/),
    ).toBeVisible();
    ...
    await page.getByRole("button", { name: "Search live prices" }).click();
    await expect(
      page.getByText("The search provider returned an error"),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("heading", { name: "Source registry" }).first(),
    ).toBeVisible();
    await shot(page, "36-source-registry");
  });

  test("competitor search and source registry on mobile 390px", async ({
    page,
  }) => {
    await installLiveSearchApiMock(page);
