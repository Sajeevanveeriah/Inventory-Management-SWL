...
test('complete synthetic workflow: load, map, validate, review, export, verify, clear', async ({
  ...
  const pipeline = page.getByRole('list', { name: 'Record counts by category' });
  ...
  await expect(page.locator('.formula-box')).toHaveText('$48.00 × 1.3 = $62.40');
  ...
  expect(saved.map((f) => path.basename(f))).toEqual([
    expect.stringMatching(/^\d{4}-\d{2}-\d{2}_.+_import-candidate_run-[A-Z0-9]{6}\.xlsx$/),
    expect.stringMatching(/_change-report_run-[A-Z0-9]{6}\.xlsx$/),
    expect.stringMatching(/_exceptions_run-[A-Z0-9]{6}\.xlsx$/),
    expect.stringMatching(/_rollback_run-[A-Z0-9]{6}\.xlsx$/),
    expect.stringMatching(/_audit-summary_run-[A-Z0-9]{6}\.txt$/),
  ]);
...
test('settings change requires confirmation and re-prices the comparison', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Review proposed changes' }).click();
  await page.getByRole('cell', { name: 'FIC-002', exact: true }).click();
  await expect(page.locator('.formula-box')).toHaveText('$48.00 × 1.4 = $67.20');
});
